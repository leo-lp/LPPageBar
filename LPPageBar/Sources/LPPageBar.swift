...
public protocol LPPageBarDelegate: NSObjectProtocol {
    ...
    func pageBar(bar: LPPageBar, didSelectedItemAt index: Int) -> Void
}

public enum LPPageSelectionLineViewMode {
    case resizesToTextWidth // LineView的宽度将和item的宽保持相等
    case fillItem // LineView的宽带将填充整个item的宽
}

public class LPPageBar: UIView {
    public weak var delegate: LPPageBarDelegate?
    ...
    public var lineViewHeight: CGFloat = 4.0 {
        didSet {
            self.updateLineViewFrame(self.selectedItemIndex)
        }
    }
    public var lineViewBottomOffset: CGFloat = 4.0 {
        didSet {
            self.updateLineViewFrame(self.selectedItemIndex)
        }
    }
    ...
    public lazy var lineViewSwitchAnimated: Bool = true
    
    /// 当lineView匹配title的文字宽度时，左右留出的空隙，lineView的宽度 = 文字宽度 + spacing
    public lazy var lineViewWidthWithSpacing: CGFloat = 0.0
    public lazy var lineViewMode: LPPageSelectionLineViewMode = .resizesToTextWidth
    
    /// pageBar边缘与第一个和最后一个item的距离
    public var leftAndRightSpacing: CGFloat = 0.0 {
        didSet {
    ...
    /// 返回已选中的item
    public var selectedItem: LPPageItem? {
        get {
    ...
    public var selectedItemIndex: Int = -1 {
        didSet {
            ...
            self.delegate?.pageBar(bar: self, didSelectedItemAt: selectedItemIndex)
        }
    }
    
    public var itemTitleColor: UIColor = UIColor.red {
        didSet {
            for item in items {
    ...
    public var itemTitleSelectedColor: UIColor = UIColor.lightGray {
        didSet {
            for item in items {
    ...
    public var itemTitleFont: UIFont = UIFont.systemFont(ofSize: 17.0) {
        didSet {
            if self.itemFontChangeFollowContentScroll {
    ...
    public var titles: [String] {
        ...
        get { return items.map { $0.title } }
    }
    
    public var items: [LPPageItem] = [] {
        willSet {
            for item in items {
        ...
        didSet {
            for item in items {
                ...
                item.addTarget(self, action: #selector(pageItemClicked), for: UIControlEvents.touchUpInside)
...
//                item.layer.borderColor = UIColor.blue.cgColor
    ...
    public var lineViewScrollFollowContent: Bool = true  // Item的选中背景是否随contentView滑动而移动
    public var itemColorChangeFollowContentScroll: Bool = true // 拖动内容视图时，item的颜色是否根据拖动位置显示渐变效果，默认为true
    public var itemFontChangeFollowContentScroll: Bool = true { // 拖动内容视图时，item的字体是否根据拖动位置显示渐变效果，默认为true
        didSet {
            self.updateItemsScaleIfNeeded()
        }
    }
    ...
    /// 更新选中背景的frame
    private func updateLineViewFrame(_ index: Int) {
    ...
    @objc private func pageItemClicked(_ item: LPPageItem) {
...
extension LPPageBar: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        guard let leftItem = leftItemOptional
            , let rightItem = rightItemOptional else {
            return
        }
